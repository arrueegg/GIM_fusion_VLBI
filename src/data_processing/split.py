...
def split_data_by_grid(data, station_counts, train_fraction=0.8, random_seed=72):
    """Split station data into training and testing sets based on grid."""
    ...
    for grid_id, count in enumerate(np.unique(data['grid'])):
        ...
        test_stations.extend(grid_data.iloc[n_train:].to_dict(orient='records'))

    return pd.DataFrame(train_stations), pd.DataFrame(test_stations)
...
def save_to_files(train_stations, val_stations, test_stations, output_dir):
    ...
    train_stations.to_csv(os.path.join(output_dir, "train.csv"), index=False, float_format='%10.2f')
    val_stations.to_csv(os.path.join(output_dir, "val.csv"), index=False, float_format='%10.2f')
    test_stations.to_csv(os.path.join(output_dir, "test.csv"), index=False, float_format='%10.2f')
...
if __name__ == "__main__":
    ...
    train_data, val_data, test_data = spatial_split(stations)
